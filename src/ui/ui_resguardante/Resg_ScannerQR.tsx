    ...
    <Pressable
      ...
      className={`rounded-lg p-4 w-full shadow-md mt-4 ${
        isSubmitting
          ...
          : 'bg-red-600 dark:bg-red-700 active:bg-red-800'
      }`}
    >
      {isSubmitting ? (
        ...
        <View className="flex-row justify-between">
          ...
          <Pressable
            ...
            className={`flex-1 rounded-lg p-4 ml-2 ${
              isSubmitting ? 'bg-gray-400' : 'bg-green-600 active:bg-green-700'
            }`}
          >
            {isSubmitting ? (
              <ActivityIndicator size="small" color="#fff" />
...
export function Resg_ScannerQR({ access_token }: { access_token: string }) {
  const insets = useSafeAreaInsets();
  const navigation =
    useNavigation<
      StackNavigationProp<RootStackParamList, 'Resg_InfoScannerQR'>
    >();
  ...
  const handleCloseAlert = () => {
    if (alertInfo.title.includes('Enviado')) {
      ...
      navigation.navigate('Resg_InfoScannerQR', { access_token, scannedData });
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text className="text-center text-xl md:text-2xl lg:text-2xl  text-gray-600 dark:text-slate-400 mt-4 px-6">
              {!isScanning
                ? 'Presiona "Empezar Escaneo" para escanear tu primer QR.'
